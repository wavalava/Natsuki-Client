...
BindCommand::BindCommand() : IMCCommand("bind", "Binds modules to different keys", "<module> <key>") {
	registerAlias("b");
}
...
bool BindCommand::execute(std::vector<std::string>* args) {
	...
	if (args->size() >= 3) {
		...
		clientMessageF("%s%s is currently bound to %s", PINK, mod->getModuleName(), Utils::getKeybindName(mod->getKeybind()));
		return true;
	}
